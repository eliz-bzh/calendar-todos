    ...
    return(
        <ol className='days-grid'>
            ...
            {currentDays.map((el, index)=>
                <li key={index} className={`calendar-day ${(new Date().toLocaleString('en-GB', {dateStyle:'medium'}) === new Date(currentDate.getFullYear(), currentDate.getMonth(), el+1).toLocaleString('en-GB', {dateStyle:'medium'}))?('current-date'):('')}`} onClick={()=>handleClick(el+1)} data-bs-toggle="offcanvas" data-bs-target="#offcanvasRight" aria-controls="offcanvasRight"><span>{el+1}</span></li>
            )}
